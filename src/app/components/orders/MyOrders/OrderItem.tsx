...
const OrderItem: React.FC<OrderItemProps> = ({ item, onViewDetails, onOrderTaken }) => {
 ...
 const styles = getTypeStyles();
  // Get the title from the right location based on item type
 const getTitle = () => {
   if (isRequest(item)) {
     return item.title || 'Untitled Request';
   } else {
     // For regular Order objects, the title is in the service object
     return item.service?.title || 'Untitled Service';
   }
 };
 ...
 return (
   <div className={`flex flex-col lg:flex-row items-start lg:items-center justify-between gap-y-4 lg:gap-x-6 border rounded-lg ${styles.border} ${styles.bg} p-4 shadow-sm hover:shadow-md transition-all transform hover:scale-[1.01] duration-200`}>
     <div className="flex flex-col lg:flex-row items-start lg:items-center gap-x-4 flex-1 px-2 lg:px-0">
       ...
       {/* Status Badge */}
       <span className={`inline-flex items-center text-xs font-medium ml-2 px-2 py-0.5 rounded-full ${statusStyle.bg} ${statusStyle.text}`}>
         {statusStyle.label}
       </span>
       ...
       <div className="flex lg:hidden space-x-2 mt-1">
         <div className={`text-xs font-medium ${styles.text} px-2 py-0.5 rounded-full bg-white/50 inline-block`}>
           {isRequest(item) && item.request_type
             ? item.request_type.charAt(0).toUpperCase() + item.request_type.slice(1)
             : 'Service'}
         </div>
